...
const { publicOrigin } = require("./_auth");
    ...
    <tr>
      <td align="center">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:600px;margin:0 auto;background:#fbf7f1;border-radius:18px;overflow:hidden;border:1px solid #eadfd4;">
          ...
          <tr>
            <td style="padding:10px 36px 8px 36px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.6;color:#64748b;">
...
async function smtpTalk(socket, cmd) {
...
function connectSmtp(host, port) {
  return new Promise((resolve, reject) => {
    ...
    if (Number(port) === 465) {
      ...
      smtpRead(socket).then((greet) => {
        if (greet.code !== 220) reject(new Error(greet.text));
        else resolve(socket);
      }).catch(onErr);
      return;
    }
    const plain = net.connect({ host, port }, async () => {
      try {
        const greet = await smtpRead(plain);
        if (greet.code !== 220) throw new Error(greet.text);
        const ehlo = await smtpTalk(plain, "EHLO bizgarh.com");
        if (ehlo.code !== 250) throw new Error(ehlo.text);
        const start = await smtpTalk(plain, "STARTTLS");
        if (start.code !== 220) throw new Error(start.text);
        const secure = tls.connect({ socket: plain, host, servername: host }, () => resolve(secure));
        secure.on("error", onErr);
      } catch (err) {
        reject(err);
      }
    });
    plain.setTimeout(20000);
    plain.on("timeout", () => plain.destroy(new Error("SMTP timeout")));
    plain.on("error", onErr);
  });
}
...
async function sendSmtp({ to, name, subject, html, text }) {
  ...
  try {
    const ehlo = await smtpTalk(socket, "EHLO bizgarh.com");
    if (ehlo.code !== 250) throw new Error(ehlo.text);
    const auth = await smtpTalk(socket, "AUTH LOGIN");
    if (auth.code !== 334) throw new Error(auth.text);
    const u = await smtpTalk(socket, Buffer.from(user, "utf8").toString("base64"));
    if (u.code !== 334) throw new Error(u.text);
    const p = await smtpTalk(socket, Buffer.from(pass, "utf8").toString("base64"));
    if (p.code !== 235) throw new Error("SMTP login failed");
    const mail = await smtpTalk(socket, `MAIL FROM:<${fromEmail}>`);
    if (mail.code !== 250) throw new Error(mail.text);
    const rcpt = await smtpTalk(socket, `RCPT TO:<${to}>`);
    if (rcpt.code !== 250) throw new Error(rcpt.text);
    const data = await smtpTalk(socket, "DATA");
    if (data.code !== 354) throw new Error(data.text);
    const done = await smtpTalk(socket, raw.replace(/^\./gm, "..") + "\r\n.");
    if (done.code !== 250) throw new Error(done.text);
    await smtpTalk(socket, "QUIT").catch(() => {});
  } finally {
    socket.end();
  }
}
