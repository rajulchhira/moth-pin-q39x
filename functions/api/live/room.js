...
import { ensureHmsRoom, hmsReady } from "../../_shared/hms.js";
...
async function handler(event) {
  ...
  try {
    const out = await ensureHmsRoom({
      kind: body.kind,
      id,
      title: body.title,
      duration: body.duration,
      asHost: Boolean(body.asHost)
    });
    return json(event, out);
  } catch (err) {
    return json(event, { ok: false, error: err.message || "Could not open live room" }, err.status || 500);
  }
}
